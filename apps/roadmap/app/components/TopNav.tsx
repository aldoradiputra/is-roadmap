...
export default function TopNav({ page, onChange, version, onOpenSearch, locale, onLocaleToggle }: Props) {
  const s = STRINGS[locale]
  const tabs: { id: Page; label: string }[] = [
    { id: 'roadmap', label: s.tabs.roadmap },
    { id: 'docs',    label: s.tabs.docs },
    { id: 'learn',   label: s.tabs.learn },
  ]
  ...
  return (
    <header style={{
      ...
      <nav style={{ display: 'flex', gap: 4, marginLeft: 16 }}>
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              <span style={{ display: 'flex', opacity: 0.85 }}>{TAB_ICONS[tab.id]}</span>
              {tab.label}
            </button>
          )
        })}
      ...
      >
        ...
        <span style={{ fontSize: 13, flex: 1 }}>{s.searchPlaceholder[page]}</span>
